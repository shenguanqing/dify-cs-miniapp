...
| 表格头/体对齐修复 | chat.vue, markdown.js | 修复表格表头与数据行列宽错位 |
...
- 表头与单元格统一列宽算法（`flex: 1 1 0` + `min-width: 0` + `box-sizing: border-box`），去掉表头 `nowrap` 改为换行，修复表头与数据行**列宽错位**
- `parseTable` 改用 `splitRow`：只去首尾边框竖线、保留行内空单元格（原 `filter(空)` 会吞掉空格列导致错位），并按表头列数对每行**补空/截断**对齐
